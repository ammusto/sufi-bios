    ...
    const loadData = async () => {
      try {
        ...
        geoJson.forEach(row => {
          ...
          if (!entry.peopleData.has(bioName)) {
            entry.peopleData.set(bioName, {
              bioId: bioId,
              contextsMap: new Map() // Map of context -> array of sources
            });
          }
          ...
          if (!personData.contextsMap.has(context)) {
            personData.contextsMap.set(context, []);
          }
          personData.contextsMap.get(context).push(source);
        ...
        const locationsArray = Array.from(locationMap.values()).map(loc => {
          ...
          // Helper to get source abbreviation
          ...
          // Convert peopleData Map to sorted array with context + source
          const peopleArray = Array.from(loc.peopleData.entries())
            .map(([name, personData]) => {
              // Format as "context [source] context [source]"
              const contextParts = [];
              
              Array.from(personData.contextsMap.entries())
                .sort(([a], [b]) => a.localeCompare(b))
                .forEach(([context, sources]) => {
                  sources.forEach(source => {
                    contextParts.push(`${context} [${getSourceAbbrev(source)}]`);
                  });
                });
      ...
      <div className="container">
        ...
        {locationData.length === 0 ? (
          ...
          <div className="mapping-map-container">
            <MapContainer
              center={[30, 50]} // Middle East center
              zoom={4}
                  ...
                  <Popup maxWidth={400} maxHeight={400}>
                    <div className="map-popup">
                      <div className="map-popup-title">
                        {location.english_transliteration}
                      </div>
                      ...
                      <div className="map-popup-sources">
                        <strong>Sources:</strong> {location.allSources.join(', ')}
                      </div>
                      ...
                      <div className="map-popup-people">
                        ...
                        <div className="map-popup-people-list">
                          {location.people.map((person, i) => (
                            <div key={i} className="map-popup-person">
                              <strong>{person.name}</strong> <br /> {person.contextString}
                            </div>
                          ))}
                        </div>
                      </div>
